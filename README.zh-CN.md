...
安装完成后，在任何 Claude Code 会话中输入 `@engram` 即可查询你的行为记忆。

### 能做什么

| 查询 | 执行逻辑 |
|------|---------|
| `@engram 上周二我在做什么？` | 读取 `daily/YYYY-MM-DD.md` 日志 |
| `@engram 我之前遇到过这个问题吗？` | 搜索 `consciousness.md` 和日志 |
| `@engram 我当前的待办事项是什么？` | 直接读取 `tasks.md` |
| `@engram 这周我表现出什么模式？` | 汇总 `patterns.md` 趋势 |
| `@engram 我的弱点是什么？` | 如实总结 `weaknesses.md` |

### 工作原理

该智能体是 Claude Code 自定义 agent（`~/.claude/agents/engram.md`），对你的记忆仓库只读访问。它使用 `Read`、`Glob` 和 `Grep` 工具搜索数据——绝不修改你的记忆。

### 安装

`scripts/install.sh` 会自动安装。手动安装：

```bash
cp agents/engram.md ~/.claude/agents/
```

重启 Claude Code，输入 `@engram` 验证是否可用。
